...
SELECT address_id,district FROM address WHERE phone IS NOT NULL;
...
/*查询film表中按标题排序的前五部电影*/
SELECT film_id,title FROM film ORDER BY title OFFSET 3 ROWS FETCH FIRST 4 ROWS ONLY;
/*查询film表中从第3行之后（即第4行）开始的4行数据*/
...
FROM payment c INNER JOIN customer a ON c.customer_id = a.customer_id INNER JOIN staff b ON c.staff_id = b.staff_id WHERE a.customer_id = 2;
/*查询客户ID 2的客户租赁数据（三表内连接）*/

SELECT a.film_id,a.title,b.inventory_id FROM film a LEFT OUTER JOIN inventory b ON a.film_id = b.film_id;
...
/*按顾客编号分组，计算总金额，并按总金额降序排列,使用聚合函数*/
...
/*并集（UNION ALL包括重复值）*/
SELECT customer_id FROM customer WHERE create_date BETWEEN '2007-02-20' AND '2007-02-21' UNION  SELECT customer_id FROM customer WHERE first_name LIKE '%er%';

/*交集*/
SELECT first_name,last_name FROM customer WHERE first_name = 'Aaron' INTERSECT
